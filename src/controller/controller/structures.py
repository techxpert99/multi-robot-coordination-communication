...
class Patch:
    ...
    def __init__(self, msg: LaserScan = None, position = None):
        if msg is None or position is None: return
...
class Map:
    ...
    def merge_patch(self, patch: Patch):
        ...
        merged_map = np.full(shape=(merged_num_rows,merged_num_cols),dtype=np.uint8,fill_value=4)

        for i in range(self.__map__.shape[0]):
            for j in range(self.__map__.shape[1]):
                if self.__map__[i,j] != 4:
...
class CriticalMap:
    ...
    def __init__(self,map: Map,critical_radius,hyper_critical_radius):
        self.__map__ = map
        self.__critical_radius__ = critical_radius
        self._hyper_critical_radius = hyper_critical_radius
    ...
    def update_critical_map(self):
        ...
        self.__bounds__ = self.__map__.__bounds__[0]-self._hyper_critical_radius,self.__map__.__bounds__[1]-self._hyper_critical_radius,self.__map__.__bounds__[2]+self._hyper_critical_radius,self.__map__.__bounds__[3]+self._hyper_critical_radius
        ...
        critical_map = np.full((self.__rows__,self.__columns__),4,np.uint8)
        delta = ceil(self._hyper_critical_radius/self.__map__.__resolution__)
        hyper_minus_critical = ceil((self._hyper_critical_radius-self.__critical_radius__)/self.__map__.__resolution__)
        hyper_plus_critical = ceil((self._hyper_critical_radius+self.__critical_radius__)/self.__map__.__resolution__)
        twice_hyper = ceil((2*self._hyper_critical_radius)/self.__map__.__resolution__)

        for i in range(rows):
            for j in range(cols):
                if matrix[i,j] == 0:
                    k,l = min(i+delta,self.__rows__-1),min(self.__columns__-1,j+delta)
                    critical_map[k,l] = 0
                    i1,i2,i3,i4 = i,min(self.__rows__-1,i+hyper_minus_critical-1),min(self.__rows__-1,i+hyper_plus_critical-1),min(self.__rows__-1,i+twice_hyper)
                    j1,j2,j3,j4 = j,min(self.__columns__-1,j+hyper_minus_critical-1),min(self.__columns__-1,j+hyper_plus_critical-1),min(self.__columns__-1,j+twice_hyper)
                    for k in range(i1,i2+1):
                        for l in range(j1,j4+1):
                            if critical_map[k,l] > 2:
                                critical_map[k,l] = 2
                    for k in range(i2+1,i3+1):
                        for l in range(j1,j2+1):
                            if critical_map[k,l] > 2:
                                critical_map[k,l] = 2
                        for l in range(j2+1,j3+1):
                            if critical_map[k,l] > 1:
                                critical_map[k,l] = 1
                        for l in range(j3+1,j4+1):
                            if critical_map[k,l] > 2:
                                critical_map[k,l] = 2
                    for k in range(i3+1,i4+1):
                        for l in range(j1,j4+1):
                            if critical_map[k,l] > 2:
                                critical_map[k,l] = 2
                elif matrix[i,j] == 3:
                    k,l = min(i+delta,self.__rows__-1),min(self.__columns__-1,j+delta)
                    if critical_map[k,l] > 3:
                        critical_map[k,l] = 3
