...
class CoCoNode:
    ...
    def __init__(self,store):
        ...
        self.desired_team_definition = {('leader',1,0.6),('mapper',3,0.4)}
        ...
        self.exchange_period = lambda: 50+random()*10 #store.get('','')
        self.exchange_timeout = lambda: 10+random()*5 #store.get('','')
        self.teammate_lost_timeout = lambda: 30+random()*10 #store.geT('','')
        self.rejected_exchanges = set()
        self.robot_lost_timeout = lambda: 40+random()*10
        self.rejection_timeout = lambda: 20+random()*5
        self.exchange_inbox = []
    ...
    def process_inbox(self):
        ...
        for message in inbox:
            if message.message_type == 'discovery':
                ...
                #print('RECEIVE',message.message_type,':',message.robot_id,'->',self.id,f'{time()%10000:.1f}')
    ...
    def stabilize_teams(self,message: CommunicationMessage):
        ...
        # peer_leader_suitability = None
        for i,id in enumerate(su_robots):
            if id == message.robot_id and su_roles[i] == 'leader':
                peer_leader_suitability = su_values[i]
                break
        # if peer_leader_suitability is None:
        #     from rich import print
        #     print('[bold red]WHOOPS????')
        #     print('[bold cyan]',message,su_robots,su_values)
        #     print()
    ...
    def send_team_change_message(self,peer,team,suitabilities):
        ...
        for _id,_rol,_su in team:
            ...
            rol.append(_rol)
            su.append(_su)
        for _sid,_srol,_sval in suitabilities:
            sval.append(_sval)
            srol.append(_srol)
            sid.append(_sid)
        msg.suitability_robots = sid
        ...
        msg.teams_su = su
        self.publisher.publish(msg)
        #print('SEND','team_change',':',self.id,'->',peer,id,rol,su)
    ...
    def finish_exchange(self,teamset):
        ...
        if teams[id_tid_map[self.id]] == self.team:
            return
    ...
    def exchange_protocol(self):
        ...
        # for robot,_,_ in self.team:
        #     if robot in self.exchange_set:
        #         self.exchange_set.remove(robot)
    ...
    def process_team_change(self):
        if self.team_change_message is None: return
        msg = self.team_change_message
        ...
        from rich import print
        # print('[bold cyan]PROCESSING TEAM CHANGE',self.id)
        # print('[bold cyan]TEAM:',team)
        # print('[bold cyan]SUITABILITIES:',suitabilities)
        # print()
        self.team = team
        self.suitabilities = suitabilities
        self.team_change_message = None

    def self_role_check(self):
        if len(self.team) == 1:
            team = {(self.id,'leader',self.role_suitabilities['leader'])}
            if self.team != team:
                self.team = team
    ...
    def callback(self):
        ...
        from rich import print
        t = Tree('[bold yellow]'+self.id)
        l = t.add('[bold red]Leader')
        m = t.add('[bold green]Mappers')
        # if (self._int is None or time()-self._int > 10):
        #     self._int = time()
        #     for id,rol,su in self.team:
        #         if rol == 'leader': l.add('[bold magenta]'+id)
        #         else: m.add('[bold blue]'+id)
        #     if self.id >= 'bot2': wait(0.05)
        #     if self.id >= 'bot3': wait(0.05)
        #     if self.id >= 'bot4': wait(0.05)
        #     print(t)
        #     if self.id == 'bot4':
        #         print('[bold cyan]_________________________________________')
        #         print()
